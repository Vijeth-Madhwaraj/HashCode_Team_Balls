...
# === USER INPUT ===
user_instruction = "Go to wikipedia and search Jane Austen then go to youtube and search for the latest sidemen video"
...
if mode == "inspect":
    inspection_prompt = (
        ...
        "- Extracts the page's HTML source for *every page it navigates to*\n"
        "- For each visited page, saves the HTML into a unique file in the tests folder\n"
        "- Use a counter (pageIndex) or timestamp in the filename, e.g. 'page_source_1.html', 'page_source_2.html'\n"
        "- Store files inside the same folder as the script\n"
